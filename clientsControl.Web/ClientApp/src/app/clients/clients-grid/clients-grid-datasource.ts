...
export class ClientsGridDataSource extends DataSource<IClient> {
  ...
  constructor(private paginator: MatPaginator, private sort: MatSort, private clientsService: ClientsService) {
    super();
    this.clientsService.getClients().subscribe(clients => { this.data = clients }, error => console.log(error));        
  }
   ...
   */
  private getPagedData(data: IClient[]) {
   ...
   * this would be replaced by requesting the appropriate data from the server.
   */
  private getSortedData(data: IClient[]) {
